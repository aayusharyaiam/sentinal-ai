    ...
    <>
      ...
      <div className={`border-t border-outline-variant/10 pt-4 transition-opacity duration-500 ${currentStage > 1 ? 'opacity-100' : 'opacity-30'}`}>
        ...
        <div className="flex items-center justify-between p-3 bg-surface-container-lowest">
          <span className={`text-sm font-semibold uppercase ${result?.status === 'BLOCKED' ? 'text-secondary' : (result?.status === 'SUSPICIOUS' ? 'text-tertiary' : 'text-primary')}`}>
            {currentStage > 1 ? (result ? result.category : "Processing...") : "Standby"}
          </span>
        ...
        <div className="flex flex-wrap gap-2">
          {currentStage > 2 && result && result.patterns.length > 0 ? (
            result.patterns.map((pat, idx) => (
              <span key={idx} className="px-3 py-1 border border-secondary text-secondary text-[10px] font-bold tracking-tight bg-secondary/10 uppercase">
                {pat}
              </span>
            ))
          ) : (
